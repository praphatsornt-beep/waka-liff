...
-- default to NULL (not set) and stay that way until an admin fills it in via
-- "✏️ แก้ไขสินค้า" (จำนวนซองต่อกล่อง). gas/Code.gs's handleConvertBoxToPack
-- refuses to convert stock for a product until this is set, so no backfill
-- is required or unsafe to skip.
