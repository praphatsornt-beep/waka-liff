...
ADMIN_CODE = st.secrets["ADMIN_CODE"]  # withdrawStock now also requires this to prove branch ownership, same as
...
with tab_central:
    ...
    if catalog.empty:
        ...
        _status_dot = {"preorder": "🟡", "ready": "🟢", "inactive": "🔴"}
        show["สถานะ"] = (
            catalog_show["status"].apply(lambda s: _status_dot.get(str(s or "").strip(), "🟢"))
            if "status" in catalog_show.columns else "🟢"
        )
        ...
        edited = st.data_editor(
            ...
            disabled=["รหัสสินค้า", "สินค้า", "สถานะ", "หมวดหมู่", "ขายแล้ว (กล่อง)", "ขายแล้ว (ซอง)", "แจ้งเตือน"],
            column_config={
