...
# ── Quick-status filter chips (top of the dense table tab) ───────────────────
QUICK_STATUS_TABS = ["ทั้งหมด", "รอแจ้งชำระ", "รอตรวจสอบ", "รอจัดเตรียม", "จัดส่งแล้ว", "ยกเลิก"]
_PENDING_REVIEW_STATUSES = {"รอตรวจ", "รอตรวจเพิ่ม", "ยอดไม่ตรง", "สลิปซ้ำ", "บัญชีไม่ตรง", "สงสัยปลอม"}
...
def quick_status_kind(slip_status: str, fulfillment: str) -> str:
    ...
    ALL_STATUS, so this is a total function (no fallback branch needed)."""
    if slip_status == "ยกเลิก":
        return "ยกเลิก"
    if slip_status == "ไม่มีสลิป":
        return "รอแจ้งชำระ"
    if slip_status in _PENDING_REVIEW_STATUSES:
        return "รอตรวจสอบ"
    # only slip_status == "ยืนยัน" reaches here
    if (fulfillment or "") in _DELIVERED_FULFILLMENTS:
        return "จัดส่งแล้ว"
    return "รอจัดเตรียม"


def items_summary_text(items: list, max_show: int = 1) -> str:
    if not items:
        return "—"
    shown = [f"{it.get('name', '')} x{it.get('qty', 1)}" for it in items[:max_show]]
    text = ", ".join(shown)
...
def slip_status_label(s: str) -> str:
    s = s or "รอตรวจ"
    emoji = {"success": "✅", "danger": "❌", "pending": "🕓"}.get(status_kind(s), "•")
    return f"{emoji} {s}"


# ย้าย (ไม่ได้เขียนใหม่) มาจากเนื้อหาใน expander ของการ์ดเดิมทุกบิต — เรียกครั้งเดียวต่อออเดอร์ที่ถูกเลือก (1 แถว) ในตารางใหม่ แทนที่จะวนสร้างทุกการ์ดเหมือนเดิม — ปุ่ม/popover/GAS call ข้างในเหมือนเดิมทุกอย่าง ไม่ได้เขียนใหม่
def render_order_detail(row) -> None:
...
tab_cards, tab_table, tab_customers = st.tabs(["📋 รายการออเดอร์", "📊 ตารางรวม", "👥 ลูกค้าทั้งหมด"])

# ── Tab: การ์ดออเดอร์ → ตารางหนาแน่นสำหรับจอคอมพิวเตอร์ (existing card-based
# workflow's actions preserved verbatim in render_order_detail()) — live actions,
# sends LINE
with tab_cards:
    ...
    quick_status_sel = st.radio(
        "สถานะด่วน", QUICK_STATUS_TABS, horizontal=True, label_visibility="collapsed", key="ord_quick_status",
    )
    if quick_status_sel != "ทั้งหมด":
        cards_filtered = filtered[filtered.apply(
            lambda r: quick_status_kind(r.get("slip_status", ""), r.get("fulfillment", "")) == quick_status_sel,
            axis=1,
        )]
    else:
    ...
    if cards_filtered.empty:
        ...
        with c1:
            st.markdown(
                f"<div style='font-size:13px;color:{TEXT2};padding-top:8px'>แสดง <strong style='color:inherit'>{len(cards_filtered)}</strong> จาก {len(df)} ออเดอร์</div>",
                unsafe_allow_html=True,
            )
        with c2:
        ...
        if sort_sel == "ใหม่สุดก่อน":
            cards_sorted = cards_filtered.sort_values("timestamp_dt", ascending=False)
        elif sort_sel == "เก่าสุดก่อน":
            cards_sorted = cards_filtered.sort_values("timestamp_dt", ascending=True)
        else:
            cards_sorted = cards_filtered.sort_values("total", ascending=False)
        ...
        # ── Dense data table (native sticky header + row-hover, both free from
        # st.dataframe's grid component — see plan doc for why this replaced the
        # old per-order card list) ──────────────────────────────────────────────
        bulk_bar_placeholder = st.empty()
        ...
        for _, row in page_df.iterrows():
            ...
            display_rows.append({
                ...
                "สถานะสลิป": slip_status_label(cur_status),
                "สถานะจัดส่ง": f"{fulfill_icon(ff_status)} {ff_status}",
                "สาขา": row.get("branch") or "—",
            })
        ...
        table_event = st.dataframe(
            ...
            column_config={"ยอดรวม": st.column_config.NumberColumn(format="฿%d")},
        )

        selected_positions = list(table_event.selection.rows) if table_event and table_event.selection else []
        selected_rows_df = page_df.iloc[selected_positions] if selected_positions else page_df.iloc[0:0]

        # ── Bulk action bar — rendered into the placeholder above the table so
        # it appears ABOVE the grid, matching the old card view's layout, even
        # though we only know the selection AFTER st.dataframe() returns ─────────
        if not selected_rows_df.empty:
            sel_pending = selected_rows_df[selected_rows_df["slip_status"].isin(["รอตรวจ", "รอตรวจเพิ่ม"])]
            sel_handover_ids = [
                r["order_id"] for _, r in selected_rows_df.iterrows()
                if r["slip_status"] == "ยืนยัน" and handover_candidates(parse_items(r.get("items_json", "")))
            ]
            sel_not_done = selected_rows_df[selected_rows_df["fulfillment"] != "รับแล้ว"]

            def _reset_table_selection():
                # ล้าง selection state ของ st.dataframe เอง หลังทำ bulk action —
                # กันแถวที่เคยเลือกไว้ (ตำแหน่งเดิม) ไปทับออเดอร์อื่นหลัง rerun
                # เพราะข้อมูล/ลำดับแถวเปลี่ยนไปแล้วจากการกระทำที่เพิ่งทำ
                st.session_state.pop("orders_table_select", None)

            with bulk_bar_placeholder.container(border=True):
                b1, b2, b3, b4, b5, b6 = st.columns([1.2, 1.3, 1.2, 1.3, 1.1, 1.1])
                with b1:
                    st.markdown(f"<div style='padding-top:8px;font-weight:600'>เลือกแล้ว {len(selected_rows_df)} ออเดอร์</div>", unsafe_allow_html=True)
                with b2:
                ...
                with b3:
                    if st.button(f"🤝 ส่งมอบ/จัดส่งที่เลือก ({len(sel_handover_ids)})", disabled=not sel_handover_ids):
                        ...
                        # handover ไม่ใช่ handoverOrder — ดู comment เดิมที่ปุ่มเดี่ยว
                        # "📤 จัดส่งแล้ว" ใน render_order_detail() สำหรับเหตุผลเต็ม
                        sel_branch_map = dict(zip(selected_rows_df["order_id"], selected_rows_df["branch"]))
                ...
                with b5:
                    if st.button("❌ ยกเลิกสลิปที่เลือก"):
                        run_bulk_action("ยกเลิกสลิปที่เลือก", selected_rows_df["order_id"], reject_slip_via_gas)
                        _reset_table_selection()
                        st.cache_data.clear()
                        st.rerun()
                with b6:
                    export_df = selected_rows_df[[
                        "order_id", "timestamp", "real_name", "phone", "branch", "total", "slip_status", "fulfillment",
                    ]].rename(columns={
                        "order_id": "เลขออเดอร์", "timestamp": "เวลา", "real_name": "ลูกค้า", "phone": "เบอร์โทร",
                        "branch": "สาขา", "total": "ยอดรวม", "slip_status": "สถานะสลิป", "fulfillment": "สถานะจัดส่ง",
                    })
                    st.download_button(
                        f"⬇️ CSV ({len(export_df)})",
                        export_df.to_csv(index=False).encode("utf-8-sig"),
                        file_name=f"orders_{datetime.now(TH_TZ).strftime('%Y%m%d_%H%M')}.csv",
                        mime="text/csv",
                    )

        # ── Detail panel — click exactly 1 row to manage that order (Streamlit
        # can't do a real hover-popover on a table cell, so this replaces the
        # spec's hover preview: click instead of hover, panel instead of popup) ──
        if len(selected_rows_df) == 1:
            st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
            with st.container(border=True):
                render_order_detail(selected_rows_df.iloc[0])
        elif len(selected_rows_df) > 1:
            st.caption(f"เลือกอยู่ {len(selected_rows_df)} ออเดอร์ — ติ๊กเลือกแถวเดียวเพื่อดูรายละเอียด/จัดการทีละออเดอร์")
        else:
            st.caption("คลิกแถวในตารางเพื่อเลือกและดูรายละเอียด")
...
with tab_table:
    st.caption(
        ...
        "— งานที่ต้องแจ้งลูกค้าจริง (อนุมัติสลิป/ส่งมอบ/ปฏิเสธ) ให้ใช้แท็บ 📋 รายการออเดอร์"
    )
